                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <form method="GET" action="{{ route('pencarian-arsip-masuk') }}" class="d-flex mb-3">
                                <input type="text" name="keyword" class="form-control me-2" placeholder="Cari berdasarkan No. Surat, Nama Surat, atau Asal Surat" value="{{ request('keyword') }}">
                                
                                <!-- Filter Bidang -->
                                <select name="bidang_id" class="form-control me-2" id="bidang_id">
                                    <option value="">Pilih Bidang</option>
                                    @foreach($bidangs as $bidang)
                                        <option value="{{ $bidang->bidang_id }}" {{ request('bidang_id') == $bidang->bidang_id ? 'selected' : '' }}>{{ $bidang->nama_bidang }}</option>
                                    @endforeach
                                </select>
                                ...
                                <select name="kategori_id" class="form-control me-2" id="kategori_id">
                                    ...
                                    @foreach($kategoris as $kategori)
                                        <option value="{{ $kategori->kategori_id }}" {{ request('kategori_id') == $kategori->kategori_id ? 'selected' : '' }}>{{ $kategori->nama_kategori }}</option>
                                    @endforeach
                                </select>
                            ...
                            @if ($arsipSuratMasuk->count() > 0)
                                <table id="arsip_surat_masuk" class="table table-hover table-bordered table-striped dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                    <thead>
                                        <tr>
                                            <th>No</th>
                                            <th>No. Surat</th>
                                    ...
                                    <tbody>
                                        @foreach ($arsipSuratMasuk as $index => $arsip)
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="{{ route('arsip_masuk.show', $arsip->surat_masuk_id) }}" class="btn btn-info btn-sm">Detail</a>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                ...
                                <div class="d-flex justify-content-center my-3">
                                    <nav aria-label="Page navigation example">
                                        <ul class="pagination">
                                            ...
                                            @else
                                                <li class="page-item"><a class="page-link" href="{{ $arsipSuratMasuk->previousPageUrl() }}">Previous</a></li>
                                            @endif

                                            <!-- Page Number Links -->
                                            @foreach ($arsipSuratMasuk->getUrlRange(1, $arsipSuratMasuk->lastPage()) as $page => $url)
                                                <li class="page-item {{ $page == $arsipSuratMasuk->currentPage() ? 'active' : '' }}">
                                            ...
                                            @if ($arsipSuratMasuk->hasMorePages())
                                                <li class="page-item"><a class="page-link" href="{{ $arsipSuratMasuk->nextPageUrl() }}">Next</a></li>
                                            @else
